...
class SegmentedConnectionManager:
    ...
    async def broadcast_segmented(self, payload: str, role_filter: Optional[str] = None):
...
def verify_password(password: str, hashed: str) -> Union[bool, int]:
    """Verify password. Returns iteration count if successful, otherwise False."""
    if not hashed: return False
    # Support plain text for legacy migration
    if "$" not in hashed: return 100000 if password == hashed else False
    
    try:
        parts = hashed.split("$")
        if len(parts) == 2:
            salt, key_hex = parts
            # Try our standard iteration counts in descending order
            counts = [100000, 50000, 29000, 10000, 2000]
            for iters in counts:
                try:
                    new_key = hashlib.pbkdf2_hmac('sha256', password.encode('utf-8'), salt.encode('utf-8'), iters)
                    if new_key.hex() == key_hex:
                        return iters
                except Exception:
                    continue
        elif len(parts) == 4 and parts[0] == "pbkdf2_sha256":
            # Support for standard PBKDF2 format: pbkdf2_sha256$iters$salt$key
            _, iters_str, salt, key_hex = parts
            iters = int(iters_str)
            new_key = hashlib.pbkdf2_hmac('sha256', password.encode('utf-8'), salt.encode('utf-8'), iters)
            return iters if new_key.hex() == key_hex else False
...
async def login(data: LoginIn):
    ...
    # Returns the successful iteration count instead of just a boolean
    verified_iters = await asyncio.to_thread(verify_password, data.password, user['password'])
    if verified_iters is False:
        raise HTTPException(status_code=401, detail="Invalid credentials.")
    
    # Auto-upgrade hash to 100k iterations if verified count is different from our standard or if plain text
    should_update = (verified_iters != 100000) or ("$" not in user['password'])
...
class SupportIn(BaseModel):
    ...
    subject: str
    message: str
    category: Optional[str] = "Report" # "Report", "Contact", "Account"
...
@app.get("/buses")
async def get_today_buses_alias(service_date: Optional[str] = None, route: Optional[str] = None, stop_id: Optional[str] = None):
                ...
                if stop_id:
                    ...
                    sql = """
                        SELECT 
                            ...
                            (SELECT MIN(sched_departure) FROM trip_stop_times WHERE trip_id = t.id) as first_departure,
                            r.name as label
                        FROM trips t
                        JOIN routes r ON r.id = t.route_id
...
@app.get("/api/bus/track/{bus_no}")
async def track_bus_by_number(bus_no: str):
    conn = get_conn()
    try:
        with conn.cursor() as cur:
            now = datetime.now()
            # 1. Find latest active trip_id for this bus_no today
            sql = """
                SELECT t.id as trip_id, r.name as route_name, b.label as bus_label
                FROM trips t 
                JOIN buses b ON b.id = t.bus_id 
                JOIN routes r ON r.id = t.route_id
                WHERE b.bus_no = %s AND t.service_date = %s
                ORDER BY t.id DESC LIMIT 1
            """
            cur.execute(sql, (bus_no, now.date()))
            trip = cur.fetchone()
            
            if not trip:
                return {"ok": False, "message": "No active trip found for this bus today"}
            
            # 2. Get latest GPS position
            sql_gps = """
            ...
            """
            cur.execute(sql_gps, (trip['trip_id'],))
            pos = cur.fetchone()
            ...
            return {
                ...
                "data": {
                    ...
                    "last_seen": str(pos['ts']) if pos else None,
                    "status": "Running" if pos else "Scheduled"
                }
            }
    finally:
        release_conn(conn)
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            AND tst.trip_id = (
                ...
                ORDER BY t2.service_date DESC, t2.id DESC LIMIT 1
            )
            ORDER BY tst.stop_order
            """
...
def search_suggestions(q: str = Query(..., min_length=2), reg_no: Optional[str] = None, role: str = "student"):
    """
...
async def routes_search(
    ...
    # --- AUTH / RECENT SEARCH LOGGING ---
    async def log_search_task(rno, rl, f_stop, t_stop):
        if not rno: return
        conn = get_conn()
        try:
            with conn.cursor() as cur:
                # 1. Find user_id from reg_no
                cur.execute("SELECT id FROM users WHERE reg_no = %s LIMIT 1", (rno,))
                user = cur.fetchone()
                if not user: return
                
                # 2. Add to recent_searches (simplified)
                cur.execute(
                    "INSERT INTO recent_searches (from_name, to_name, role, ts, user_id) VALUES (%s, %s, %s, %s, %s)",
                    (f_stop, t_stop, rl, get_now_ist(), user['id'])
                )
                conn.commit()
        except: pass
    ...
    if reg_no:
        background_tasks.add_task(log_search_task, reg_no, role, from_stop, to_stop)
        ...
        with conn.cursor() as cur:
            ...
            def run_search(d):
                ...
                SELECT
                  ...
                  DATE_FORMAT(fs.sched_departure, '%%H:%%i') AS from_departure,
                  DATE_FORMAT(ts.sched_arrival, '%%H:%%i') AS to_arrival,
                  ...
                  (SELECT s4.name FROM trip_stop_times tst4 JOIN stops s4 ON s4.id=tst4.stop_id WHERE tst4.trip_id=t.id ORDER BY tst4.stop_order ASC LIMIT 1) as route_start_name,
                  (SELECT s5.name FROM trip_stop_times tst5 JOIN stops s5 ON s5.id=tst5.stop_id WHERE tst5.trip_id=t.id ORDER BY tst5.stop_order DESC LIMIT 1) as route_end_name
                FROM trips t
                JOIN routes r ON r.id=t.route_id
                LEFT JOIN buses b ON b.id=t.bus_id
                ...
                """
                cur.execute(sql, from_ids + to_ids + [d])
                return cur.fetchall()
            ...
            if not rows and from_stop and to_stop:
                ...
                for v in vehicles:
                    ...
                    rows.append({
                        ...
                        "from_departure": get_now_ist().strftime("%H:%M"),
                        "to_arrival": (get_now_ist() + timedelta(minutes=20)).strftime("%H:%M"),
                        "duration_minutes": 20,
...
async def get_trip_timeline(trip_id: int, ext_trip_id: Optional[str] = None, from_stop_id: Optional[str] = None, to_stop_id: Optional[str] = None):
    """
    ...
    try:
        with conn.cursor() as cur:
            ...
            if trip_id > 0:
                ...
                return {"ok": False, "detail": "No trip_id or ext_trip_id provided"}

            stops = cur.fetchall()
            
            if not stops and ext_trip_id:
                # Fallback: Synthesis for simulated vehicles (Guess route)
                guess_rt = ext_trip_id.replace("Bus", "").replace(" ", "")
                # Find most recent trip ID for this route to use as a template for stop sequence
                cur.execute("""
                    SELECT tst.stop_id, s.name as stop_name, s.lat, s.lng, 
                           tst.stop_order, tst.sched_arrival, tst.sched_departure,
                           NULL as actual_arrival, NULL as actual_departure, 'Scheduled' as status,
                           0 as bus_id, %s as bus_no
                    FROM trip_stop_times tst
                    JOIN stops s ON s.id = tst.stop_id
                    JOIN trips t ON t.id = tst.trip_id
                    JOIN routes r ON r.id = t.route_id
                    WHERE (r.ext_route_id = %s OR r.name LIKE %s OR r.id = %s)
                    AND tst.trip_id = (
                        SELECT t2.id FROM trips t2 
                        WHERE t2.route_id = r.id 
                        ORDER BY t2.service_date DESC, t2.id DESC LIMIT 1
                    ...
                    ORDER BY tst.stop_order ASC
                """, (ext_trip_id, guess_rt, f"%{guess_rt}%", int(guess_rt) if guess_rt.isdigit() else 0))
                stops = cur.fetchall()
...
async def traccar_push(
    ...
    """
    Endpoint for Traccar Client / GPS Logger apps.
    Format: /api/gps/traccar?id=DEVICE_ID&lat=LAT&lon=LON&spd=SPEED&hdg=BEARING
    Hardened to handle malformed inputs (like typos or unreplaced placeholders) gracefully.
    """
    ...
    # Manual parsing to avoid 422 errors
    lat_f = to_float(lat, None)
    lon_f = to_float(lon, None)
    spd_f = to_float(spd, 0.0)
    hdg_f = to_float(hdg, 0.0)

    now = get_now_ist()
    
    # If no coordinates, we can't do much, but we return 200 to keep the app happy
    if lat_f is None or lon_f is None:
        print(f"⚠️ GPS Logger warning: Missing or invalid coordinates from {id} (lat: {lat}, lon: {lon})")
        return {"status": "ok", "msg": "ignored_missing_coords"}
        ...
        def save_traccar_to_db():
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    """
                    cur.execute(sql, (id, id, now, lat_f, lon_f, spd_f, hdg_f, "Traccar Route", trip_id))
                    conn.commit()
            except Exception as e:
...
async def owntracks_push(request: Request, background_tasks: BackgroundTasks, id: Optional[str] = None):
    """
...
@app.get("/trips/{identifier}/latest-gps")
@app.get("/api/trips/{identifier}/latest-gps")
async def get_specific_latest_gps(identifier: str):
    """
    Returns the latest GPS point for a specific vehicle (Bus1, 1, etc.)
    Used by the frontend LiveTrackingViewModel for high-frequency polling.
    """
    # 1. Try fetching from Redis first (fastest)
    vid = identifier.replace("Bus", "").replace(" ", "")
    cached = await redis_get(f"gps:latest:{vid}")
    
    if not cached:
        # Fallback to normalized Bus name for keys like 'gps:latest:Bus1'
        cached = await redis_get(f"gps:latest:{identifier}")
    
    if cached:
        try:
            vehicle = json.loads(cached)
            return {"ok": True, "data": {
                "lat": float(vehicle.get("lat", 0)),
                "lng": float(vehicle.get("lon", 0)),
                "speed": float(vehicle.get("spd", 0)),
                "heading": float(vehicle.get("hdg", 0)),
                "ts": vehicle.get("ts")
            }}
        except:
            pass
    ...
    try:
        with conn.cursor() as cur:
            # Join with trips to handle identifier as trip_id OR bus_no
            sql = """
                SELECT gp.lat, gp.lng, gp.speed, gp.heading, gp.ts 
                FROM gps_points gp
                LEFT JOIN trips t ON t.id = gp.trip_id
                LEFT JOIN buses b ON b.id = t.bus_id
                WHERE gp.ext_vehicle_id = %s 
                   OR b.bus_no = %s 
                   OR t.ext_trip_id = %s
                   OR gp.trip_id = %s
                ORDER BY gp.ts DESC LIMIT 1
            """
            tid = int(identifier) if identifier.isdigit() else 0
            cur.execute(sql, (identifier, identifier, identifier, tid))
            row = cur.fetchone()
            if row:
                return {"ok": True, "data": {
                    "lat": float(row['lat']),
        ...
        print(f"Error in get_specific_latest_gps: {e}")
    finally:
        release_conn(conn)

    return {"ok": False, "data": None, "error": "No GPS data found for this trip"}
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    for trip in trips:
                        ...
                        prev_stop = stops[0]
                        for s in stops:
                            # Convert sched_arrival (timedelta from pymysql TIME column) to datetime for comparison
                            arr = s['sched_arrival']
                            if isinstance(arr, timedelta):
                                total_secs = int(arr.total_seconds())
                            else:
                                h, m, sec = map(int, str(arr).split(':'))
                                total_secs = h * 3600 + m * 60 + sec
                            sched_dt = datetime.combine(today, datetime.min.time()) + timedelta(seconds=total_secs)
                            if sched_dt > now:
                                target_stop = s
                                break
                            prev_stop = s
...
def list_recent_searches(role: str = "student", limit: int = 5, user_id: Optional[int] = None):
    conn = get_conn()
    try:
        with conn.cursor() as cur:
            if user_id:
...
def admin_get_drivers():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT id, first_name, last_name, email, mobile_no, role FROM users WHERE role = 'driver'")
            return {"ok": True, "data": serialize_rows(cur.fetchall())}
    finally: release_conn(conn)
...
async def voice_intent(request: VoiceTranscriptRequest):
    """
    Parses a voice transcript from the iOS app using Gemini.
    Returns a structured command for the app to execute.
    """
    try:
        data = voice_agent.parse_voice_intent(request.transcript)
        return data
    except Exception as e:
        print(f"Voice Assistant Error: {e}")
        return {
            "command": "STATUS",
            "speech_response": "Sorry, I had trouble processing your voice request."
        }

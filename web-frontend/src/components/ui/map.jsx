...
const MAPTILER_KEY = "BPXuV9PH3LNsrPELzp3Q";
...
const defaultStyles = {
  ...
  city: `https://api.maptiler.com/maps/streets-v2/style.json?key=${MAPTILER_KEY}`,
};
...
const Map = forwardRef(function Map(
  {
    ...
    theme: themeProp,
    styles,
    mapType, // NEW: 'city', 'terrain', 'satellite'
    projection,
    viewport,
    onViewportChange,
    ...props
  ...
  const mapStyles = useMemo(() => ({
    ...
    terrain: styles?.terrain ?? defaultStyles.terrain,
    satellite: styles?.satellite ?? defaultStyles.satellite,
    city: styles?.city ?? defaultStyles.city,
  }), [styles]);
  ...
  useEffect(() => {
    ...
    const initialStyle = mapType ? mapStyles[mapType] : (resolvedTheme === "dark" ? mapStyles.dark : mapStyles.light);
  ...
  // Handle style/mapType change
  useEffect(() => {
    if (!mapInstance) return;

    const newStyle = mapType ? mapStyles[mapType] : (resolvedTheme === "dark" ? mapStyles.dark : mapStyles.light);
  ...
  }, [mapInstance, resolvedTheme, mapStyles, mapType, clearStyleTimeout]);
